...
**`AUTOMATION.md` 只負責兩件事：**（1）要在 GitHub 哪裡填哪些 **Secrets**；（2）排程（每日摘要、每週專題、每月內容審計）各自做什麼。
...
4. **若要「每週專題文章」、每月內容審計，或「摘要英→繁」**：新增一筆 Secret，**Name** 填 `MINIMAX_API_KEY`，**Secret** 貼上你在 MiniMax 平台複製的金鑰；存檔即可。`daily-digest`、`weekly-article` 與 `monthly-content-audit` 會共用此金鑰（摘要翻譯、週報撰寫、每月內容審計）。其餘 `MINIMAX_MODEL`、`MINIMAX_API_BASE` **可不填**（用預設即可）。**若本機或 Actions 出現 `401 invalid api key`，金鑰已確認無誤時**，請再新增 Secret **`MINIMAX_API_BASE`**，值設為 **`https://api.minimaxi.com/v1`**（與本機 PowerShell `$env:MINIMAX_API_BASE` 相同效果）。
5. 到倉庫 **Actions** 分頁，可手動點 **Run workflow** 測試 **Daily digest**、**Weekly featured article** 或 **Monthly content audit**。這三個排程成功後會 **直接更新 `main` 並部署 Pages**。
...
| Secret | 必填 | 說明 |
|--------|------|------|
| `MINIMAX_API_KEY` | 週報與每月審計必填；**摘要若要英→繁亦必填**（同一筆） | [MiniMax 平台](https://platform.minimax.io) 取得之 API Key；Bearer 驗證。 |
...
未設定 `MINIMAX_API_KEY` 時，`automation/weekly-article.mjs` 與 `automation/monthly-content-audit.mjs` 會**安靜結束**（exit 0），不會更新內容檔案。`build-digest.mjs` 則不會翻譯，摘要 JSON 內標題／摘要維持 RSS 原文。
...
| [`.github/workflows/monthly-content-audit.yml`](.github/workflows/monthly-content-audit.yml) | 每月 28 日 UTC 03:00（約香港 11:00） | 執行 `automation/monthly-content-audit.mjs`：消化當月週報與 RSS 摘要，更新家長月報、首頁／服務／資源／專欄標記區塊、SEN 專頁補充 FAQ，再套用 SEO／IndexNow，**直接 push `main` 並部署**。不改收費、電話、預約表。 |
| [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml) | `main` push；digest／週報／每月審計成功後 | GitHub Pages 部署至 `www.leisure.org.hk`。 |

digest、週報與每月審計 workflow 皆支援 **Actions 手動執行**（`workflow_dispatch`）。
...
3. **每月內容審計**：會改常駐說明與 FAQ 補充，但凍結收費／聯絡；建議抽查 `data/monthly-audit-log.json` 與當月 `monthly-insights-YYYY-MM.html`。
4. **著作權**：摘要僅使用 RSS 之標題與短描述並連結至原文；站內不轉載全文。週報為模型依摘要**改寫與在地化**之文稿，文末列出參考連結。
...
- `monthly-content-audit.mjs` 在模型失敗時會改用本機摘要仍寫出月報；未設金鑰則略過。
